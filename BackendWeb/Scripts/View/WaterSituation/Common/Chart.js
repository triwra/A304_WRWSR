class RealtimeReservoirInformationChart {
    X_start; X_end;
    chart_title_txt = "";
    constructor(sectionId, DataSet, labelName) {
...
/*            top: '25%',*/
    ...
    setXAxis(option) {
        option.xAxis = {
            ...
            scale: true,
            boundaryGap: true,
            data: Enumerable.From(this.DataSet).Select(function (x) { return x[0]; }).ToArray(),
    ...
    setSeries(option, DataSet, labelName = "") {
        ...
        if (typeof DataSet != 'undefined') {
            console.log(Enumerable.From(DataSet).Select(function (x) { return x[1]; }).ToArray());
            option.series.push(
                {
                    name: labelName,
...
class ReservoirWaterStorageChart {
    ...
    setLegend(option, DataSet) {
        if (typeof DataSet.linedata != 'undefined') {
            let tempdata = Enumerable.From(DataSet.linedata)
                .Select(function (x) {
                    return x.name
                })
                .ToArray();
    ...
    setSeries(option, DataSet, labelName = "") {
        ...
        if (typeof DataSet != 'undefined') {
            for (let i = 0; i < DataSet.linedata.length; i++) {
                console.log(DataSet.linedata[i].data);
                let linedata = DataSet.linedata[i];
                option.series.push(
                    {
                        name: linedata.name,
                        data: Enumerable.From(linedata.data).Select(function (x) { return x[1]; }).ToArray(),
                        z: 9999,
                        type: 'line',
                        //smooth: true,
                        symbol: 'none',
            ...
            option.series.push(
                {
                    name: "嚴重下限",
                    data: DataSet.SeriousLowerLimitdata,
                    z: 9999,
                    type: 'line',
                    itemStyle: {
                        color: '#FF0000',
    ...
    setTooltip(option, DataSet) {
        option.tooltip = {
            ...
            formatter: function (params) {
                ...
                return `<div>民國${year}年</div><div>排名：${params[0].axisValue}</div> <div>蓄水量：${Math.round(params[0].value).toLocaleString('en-US') }</div>`;
            },
            axisPointer: {
                type: 'cross',
                animation: false,
                ...
                [
                    {
                        ...
                        markLine: {
                            data: [
                                ...
                                [
                                    {
                                        name: `同期平均蓄水量:${Math.round(avgval).toLocaleString('en-US') }`,
                                        label: {
                                            show: true,
                                            color: '#000',
                                            position: 'insideEndTop',
                                    ...
                                    {
                                        name: `同期蓄水量:${Math.round(avgval).toLocaleString('en-US') }`,
                                        label: {
                                            show: true,
                                            color: '#000',
                                            position: 'insideEndTop'
                                ...
                                [
                                    {
                                        name: `${thisYearData[0][0]}同期蓄水量:${Math.round(thisYearData[0][1].value).toLocaleString('en-US')}　佔歷年平均${(thisYearData[0][1].value / avgval * 100).toFixed(0)}%`,
                                        label: {
                                            show: true,
                                            color: '#000',
