...
$(document).ready(function () {
    ...
    initDatePicker();
    ajax = new WaterStorageRankingAjax(ajax_src);
    DoReservoirWaterStorageRankProcess();

});

//DoRainfallRankProcess
function DoReservoirWaterStorageRankProcess() {
    $('#loading-part').removeClass('hide');
    ...
    if ($('[name="dataRangeRadio"]:checked').val() == 'all') {
        $.when(
            ajax.GetDateReservoirEffectStorageRankData(BoundaryID, date.Start),
        ).done(EffectStorageProcess);
    } else {
        ...
        $.when(
            ajax.GetDateReservoirEffectStorageRankData(BoundaryID, date.Start, DataRangeStartY, DataRangeEndY),
        ).done(EffectStorageProcess);
    }

}

function EffectStorageProcess(r1) {
    console.log(dataTool.string2Json(r1));

    let Rank = dataTool.string2Json(r1);
    //let Avg = dataTool.string2Json(r1[0])[0];
 
    EffectStorageTableDataProcess(Rank);
    setRankChart(Rank, 'Rainfall');
    //$('#loading-part').addClass('hide');
}
function EffectStorageTableDataProcess(data) {
    ...
    setEffectStorageTable(data, hasPassData);
}
function setEffectStorageTable(data, hasPassData = false) {
    console.log(data);
    ...
    let tableCol = [
        ...
        },
        {
            sortable: true,
            field: 'value',
            title: MDStartDate + '蓄水量<br>(萬立方公尺)',
            align: 'center',
            valign: 'middle',
    ...
    $('#datatable th[data-field="value"] .th-inner').html(MDStartDate + '蓄水量<br>(萬立方公尺)')
    $('#loading-part').addClass('hide');
}
...
$('#Query').click(function () {
    ...
    switch (parseInt($('#BoundaryType option:selected').val())) {
        case 1: DoReservoirWaterStorageRankProcess(); break;
