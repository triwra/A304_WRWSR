...
$(document).ready(function () {
    ...
    initDatePicker();
    ajax = new RainfallRanking(ajax_src);
    DoReservoirRainRankProcess();

});

//DoRainfallRankProcess
function DoReservoirRainRankProcess() {
    $('#loading-part').removeClass('hide');
    ...
    if ($('[name="dataRangeRadio"]:checked').val() == 'all') {
        $.when(
            ajax.GetDateRangeReservoirRainRankData(BoundaryID, date.Start, date.End),
        ...
        $.when(
            ajax.GetDateRangeReservoirRainRankData(BoundaryID, date.Start, date.End, DataRangeStartY, DataRangeEndY),
        ).done(RainfallProcess);
    }

}

function DoIARainRankProcess() {
    $('#loading-part').removeClass('hide');
    //BoundaryID = $('#BoundaryID option:selected').text();
    let BoundaryID = $('#BoundaryID option:selected').val();
    //console.log(ReservoirTxt, ReservoirVal);
    ...
    if ($('[name="dataRangeRadio"]:checked').val() == 'all') {
        $.when(
            ajax.GetDateRangeIAGridRainRankData(BoundaryID, date.Start, date.End),
        ).done(RainfallProcess);
    } else {
        ...
        //console.log(BoundaryID, date.Start, date.End, DataRangeStartY, DataRangeEndY);
        $.when(
            ajax.GetDateRangeIAGridRainRankData(BoundaryID, date.Start, date.End, DataRangeStartY, DataRangeEndY),
        ).done(RainfallProcess);
    }

}

function RainfallProcess(r1) {
    //console.log(dataTool.string2Json(r1));
    let Rank = dataTool.string2Json(r1);
    //let Avg = dataTool.string2Json(r1[0])[0]; 
    RainfallTableDataProcess(Rank);
    setRankChart(Rank, 'Rainfall');
    //$('#loading-part').addClass('hide');
}

function RainfallTableDataProcess(data) {
...
function setRainfallTable(data, hasPassData = false) {
    ...
    $('#datatable th[data-field="value"] .th-inner').html(MDStartDate + '至' + MDEndDate + '<br>累計雨量(mm)')
    $('#loading-part').addClass('hide');
}
...
$('#Query').click(function () {
    ...
    switch (parseInt($('#BoundaryType option:selected').val())) {
        case 1: DoReservoirRainRankProcess(); break;
        case 3: DoIARainRankProcess(); break;
